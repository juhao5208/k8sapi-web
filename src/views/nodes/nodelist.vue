  ...
  <div>
    <el-table
      ...
      <el-table-column label="内存" width="100" align="center">
        <template slot-scope="scope">
          {{ Math.round(scope.row.Usage.Memory * 100) }}% /
      ...
      <el-table-column label="操作" width="100" align="center">
        <template slot-scope="scope">
          <router-link icon="el-icon-edit" :to="{name:'Nodedetail',
              params:{node:scope.row.Name}}">
            <el-link>编 辑 <i class="el-icon-edit-outline"></i></el-link>
          </router-link>
          <router-link :to="{name:'Nodeshell',
              params:{node:scope.row.Name}}">
            <el-link>远 程<i class="el-icon-s-platform el-icon--right"></i></el-link>
          </router-link>
        </template>
      </el-table-column>
    </el-table>
